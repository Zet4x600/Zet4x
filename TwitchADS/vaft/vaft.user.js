--- START OF FILE vaft.user.js ---

// ==UserScript==
// @name         TwitchADS (vaft)
// @namespace    https://github.com/Zet4x600/TwitchADS
// @version      1.2
// @description  No interruptions & HD quality fix
...
// @author       https://github.com/Zet4x600/
// @match        *://*.twitch.tv/*
...
(function() {
                                        ...
                                        if (settingsCog) {
                                            ...
                                            if (lowQuality) {
                                                ...
                                                if (e.data.value != null) {
                                                    // MODIFICATO: Inizio della logica di qualità migliorata
                                                    ...
                                                    // Mappa le risoluzioni in modo più granulare
                                                    if (e.data.value.includes('auto')) {
                                                        qualityToSelect = 0;
                                                    } else if (e.data.value.includes('source') || e.data.value.includes('1080p') || e.data.value.includes('960p') || e.data.value.includes('936p') || e.data.value.includes('900p')) {
                                                        qualityToSelect = 1;
                                                    } else if (e.data.value.includes('864p') || e.data.value.includes('822p') || e.data.value.includes('720p')) {
                                                        qualityToSelect = 2;
                                                    } else if (e.data.value.includes('480p')) {
                                                        qualityToSelect = 3;
                                                    } else if (e.data.value.includes('360p')) {
                                                        qualityToSelect = 4;
                                                    } else if (e.data.value.includes('160p')) {
                                                        qualityToSelect = 5;
                                                    }
                                                    // MODIFICATO: Fine della logica di qualità migliorata
                                                }
    ...
    async function getStreamForResolution(streamInfo, resolutionInfo, encodingsM3u8, fallbackStreamStr, playerType, realFetch) {
        ...
        if (streamM3u8Response.status == 200) {
            ...
            // MODIFICATO: Rimossa la chiamata che causa l'interruzione (cambio di qualità).
            // postMessage({
    ...
    async function processM3U8(url, textStr, realFetch, playerType) {
        ...
        if (haveAdTags) {
            ...
            if (WasShowingAd) {
                ...
                WasShowingAd = false;
                // MODIFICATO: Rimosse le chiamate che causano l'interruzione (ripristino qualità e pausa/play).
                // postMessage({
                //     key: 'ForceChangeQuality',
                //     value: 'original'
                // });
